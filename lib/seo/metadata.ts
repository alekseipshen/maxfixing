...
export function generatePageMetadata(params: SEOParams): Metadata {
  ...
  if (city && brand && appliance) {
    ...
    const brandName = formatBrandName(brand);
    const applianceName = formatApplianceName(appliance);
    title = `${brandName} ${applianceName} Repair Los Angeles & Orange County | Expert ${brandName} Service`;
    ...
    const brandName = formatBrandName(brand);
    title = `${brandName} Appliance Repair Los Angeles & Orange County | ${SITE_NAME}`;
    description = `Authorized ${brandName} appliance repair across Los Angeles and Orange County. Factory-trained technicians, all major ${brandName} appliances. Same-day service. Call ${PHONE}!`;
  } else if (appliance) {
    const applianceName = formatApplianceName(appliance);
    title = `${applianceName} Repair Los Angeles & Orange County | Same-Day Service | ${SITE_NAME}`;
    description = `Expert ${applianceName} repair across Los Angeles and Orange County. Certified technicians, same-day service, all major brands. Call ${PHONE} for professional ${applianceName} repair!`;
  } else {
    // Homepage
