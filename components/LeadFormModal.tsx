    ...
    try {
      ...
      const response = await fetch('/api/submit-lead', {
        ...
        body: JSON.stringify({
          ...
          phone: cleanedPhone,
          email: data.email,
          message: `${cleanText(data.message || '')}\n\nAddress: ${cleanText(data.street)}${data.apartment ? ', ' + cleanText(data.apartment) : ''}, ${cleanText(data.city)}, CA ${data.zipCode}\n\nPreferred: ${isoDate} (${timeSlotLabel})`,
      ...
      <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto bg-white rounded-2xl shadow-2xl">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between rounded-t-2xl z-10">
          <h2 className="text-xl font-bold text-gray-900">
            {submitStatus === 'success' ? 'Thank You!' : 'Max Fixing - Appliance Service'}
          </h2>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-4">
          ...
          {currentStep === 1 && submitStatus !== 'success' && (
            <div className="space-y-6">
              <div>
                <h3 className="text-2xl font-bold text-gray-900 mb-2">Welcome to Max Fixing</h3>
